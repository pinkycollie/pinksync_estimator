...
export interface UbuntuSFTPConfig {
  host: string;
  port: number;
  username: string;
  password?: string;
...
export function createUbuntuClientFromIntegration(integration: Integration): SFTPClient | null {
  if (!integration || integration.type !== 'ubuntu') {
    return null;
  }
  
  try {
    const config = integration.config as UbuntuSFTPConfig;
    
    if (!config || !config.host || !config.username) {
      console.error('Invalid Ubuntu integration config');
      return null;
    }
    ...
    console.error('Error creating Ubuntu SFTP client:', error);
    return null;
  }
}

export async function testUbuntuConnection(client: SFTPClient, config: UbuntuSFTPConfig): Promise<{ success: boolean; message?: string }> {
  try {
    ...
    return { 
      success: true, 
      message: `Connected to Ubuntu at ${config.host} as ${config.username}` 
    };
  } catch (error: any) {
    console.error('Ubuntu connection test failed:', error);
    ...
    return { 
      success: false, 
      message: error.message || 'Failed to connect to Ubuntu server' 
    };
  }
}
...
export function determineFileCategory(fileType: string, fileName: string): FileCategory {
  ...
  // Code types
  if (['.js', '.ts', '.py', '.java', '.c', '.cpp', '.html', '.css', '.php', '.rb', '.go', '.rs', '.sh', '.bash'].includes(fileExtension)) {
  ...
  // Note types
  if (['.md', '.markdown', '.note', '.rst'].includes(fileExtension)) {
...
export async function synchronizeFromUbuntu(
  client: SFTPClient,
  config: UbuntuSFTPConfig,
  userId: number
  ...
  try {
    ...
    const basePath = config.basePath || '/home/' + config.username;
    ...
    for (const fileInfo of remoteFiles) {
      try {
        ...
        console.error(`Error processing Ubuntu file ${fileInfo.name}:`, fileError);
    ...
    console.error('Error synchronizing from Ubuntu:', error);
...
async function listFilesRecursively(client: SFTPClient, remotePath: string, fileList: any[] = []): Promise<any[]> {
  try {
    ...
    for (const item of items) {
      ...
      // Skip . files/directories and node_modules
...
export async function downloadUbuntuFile(client: SFTPClient, config: UbuntuSFTPConfig, filePath: string): Promise<Buffer | null> {
  try {
    await client.connect({
      host: config.host,
      port: config.port || 22,
    ...
    console.error('Error downloading file from Ubuntu:', error);
