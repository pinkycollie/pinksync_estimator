...
class AutomationService {
  ...
  async executeWorkflow(
    ...
    try {
      ...
      const steps = workflow.workflowSteps ? JSON.parse(workflow.workflowSteps) : [];
  ...
  private async getFileById(fileId: number): Promise<any> {
    try {
      // Get the file from the database
      const workflow = await storage.getAutomationWorkflow(fileId);
      return workflow;
    } catch (error) {
  ...
  private async executeDatabaseRead(modelType: string, query: any, options: any, executionData: any): Promise<any> {
    ...
    switch (modelType) {
      case 'file':
        if (query.id) {
          return await storage.getFileById(query.id);
      ...
      case 'recommendation':
        if (query.id) {
          return await storage.getRecommendationById(query.id);
